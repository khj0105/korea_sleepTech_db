...
# : 재고 수량 자동 조정, 포인트 자동 적립 '자동화된 비즈니스 로직 처리'
...
create trigger 트리거이름
	트리거시점(종류: after, before) 이벤트 종류(insert, update, delete)
    on 테이블이름
    for each row
begin
	-- 트리거 실행 코드
...
-- 트리거 생성
drop trigger if exists myTriggeer; # 프로시저나 트리거 drop 가능
...
# - 실행 조건: Begin ~ Commit 또는 Callback으로 직접 제어
